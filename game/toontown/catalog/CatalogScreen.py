# This module implements the catalog interface, presenting and updating player controls and
...
class CatalogScreen(DirectFrame):
    ...
    def load(self, guiItems, guiButton, guiBack):
        ...
        self.coverTitle = DirectLabel(self.cover, relief=None, text='CATALOG', scale=0.13, pos=(0.0, 0, 0.55), text_fg=(1.0, 0.95, 0.4, 1), text_shadow=(0, 0, 0, 1), text_font=ToontownGlobals.getSignFont())
    ...
    def loadClarabelle(self):
        ...
        self.clarabelle = Toon.Toon()
        guideDNA = ToonDNA.ToonDNA()
        guideDNA.newToonRandom(7319, 'f', 1)
        self.clarabelle.setDNAString(guideDNA.makeNetString())
        self.clarabelle.setName('Catalog Guide')
        self.clarabelle.setPickable(0)
        self.clarabelle.loop('neutral')
        self.clarabelle.startBlink()
        self.clarabelle.startLookAround()
        switchboard = loader.loadModel('phase_5.5/models/estate/switchboard')
        switchboard.reparentTo(self.cRender)
        switchboard.setPosHprScale(0.35, 6.6, -3.15, 180, 0, 0, 0.75, 0.75, 0.75)
        self.clarabelle.reparentTo(self.cRender)
        self.clarabelle.setPosHprScale(-0.75, 6.6, -3.15, 180, 0, 0, 0.75, 0.75, 0.75)
    ...
    def unloadClarabelle(self):
        ...
        del self.cCam
        del self.cDr
        self.clarabelle.stopLookAround()
        self.clarabelle.stopBlink()
        self.clarabelle.delete()
        del self.clarabelle
