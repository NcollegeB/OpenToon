...
VALID_FRAME_RATE_LIMITS = (0, 30, 60, 120, 144)
VALID_MSAA_LEVELS = (0, 2, 4, 8)
VALID_ANISOTROPY_LEVELS = (1, 2, 4, 8, 16)


def _boundedNumber(value, default, minimum, maximum):
    try:
        value = float(value)
...
def _allowedInteger(value, default, allowed):
    try:
        value = int(value)
    except (TypeError, ValueError):
        value = default
...
class ToonBase(OTPBase.OTPBase):
    ...
    def __init__(self):
        ...
        if not ConfigVariableInt('ignore-user-options', 0).value:
            ...
            res = self.settings.getSetting('resolution', DEFAULT_RESOLUTION)
            vsync = bool(self.settings.getSetting('vsync', True))
            msaa = _allowedInteger(
                self.settings.getSetting('msaa', 0), 0, VALID_MSAA_LEVELS)
            anisotropy = _allowedInteger(
                self.settings.getSetting('anisotropy', 8),
                8,
                VALID_ANISOTROPY_LEVELS)
            fpsLimit = _allowedInteger(
                self.settings.getSetting('fps-limit', 60),
                60,
                VALID_FRAME_RATE_LIMITS)
            showFps = bool(self.settings.getSetting('show-fps', False))
            particles = bool(self.settings.getSetting('particles', True))
            if mode == None:
                mode = 1
            if not isinstance(res, (list, tuple)) or len(res) != 2:
                res = DEFAULT_RESOLUTION
            try:
                res = (max(640, int(res[0])), max(480, int(res[1])))
            except (TypeError, ValueError):
                res = DEFAULT_RESOLUTION
            ...
            loadPrcFileData('toonBase Settings VSync', 'sync-video %s' % vsync)
            loadPrcFileData(
                'toonBase Settings MSAA',
                'framebuffer-multisample %s\nmultisamples %s' %
                (msaa > 0, msaa))
            loadPrcFileData(
                'toonBase Settings Texture Filtering',
                'texture-anisotropic-degree %s' % anisotropy)
            loadPrcFileData(
                'toonBase Settings Frame Rate',
                'clock-mode %s\nclock-frame-rate %s' %
                ('limited' if fpsLimit else 'normal',
                 fpsLimit if fpsLimit else 60))
            loadPrcFileData(
                'toonBase Settings FPS Meter',
                'show-frame-rate-meter %s' % showFps)
            loadPrcFileData(
                'toonBase Settings Particles',
                'want-particles %s' % particles)
        ...
        cameraFov = _boundedNumber(
            self.settings.getSetting(
                'fov', ToontownGlobals.DefaultCameraFov),
            ToontownGlobals.DefaultCameraFov,
            45.0,
            90.0)
        OTPGlobals.DefaultCameraFov = cameraFov
        ToontownGlobals.DefaultCameraFov = cameraFov
        self.camLens.setFov(cameraFov)
        self.camLens.setNearFar(ToontownGlobals.DefaultCameraNear, ToontownGlobals.DefaultCameraFar)
        self.musicManager.setVolume(
            _boundedNumber(
                self.settings.getSetting('music-volume', 0.65),
                0.65,
                0.0,
                1.0))
        ...
        if bool(self.settings.getSetting('particles', True)):
    ...
    def applyRuntimeSettings(self):
        """Apply settings that Panda3D can safely change without a restart."""

        frameRateLimit = _allowedInteger(
            self.settings.getSetting('fps-limit', 60),
            60,
            VALID_FRAME_RATE_LIMITS)
        taskMgr.remove('log-measured-client-frame-rate')
        if frameRateLimit:
            globalClock.setMode(ClockObject.MLimited)
            globalClock.setFrameRate(frameRateLimit)
            self.notify.info(
                'Configured client frame-rate limit: %s FPS' % frameRateLimit)
            taskMgr.doMethodLater(
                10.0,
                self._logMeasuredFrameRate,
                'log-measured-client-frame-rate')
        else:
            globalClock.setMode(ClockObject.MNormal)
            self.notify.info('Configured client frame-rate limit: unlimited')

        self.setFrameRateMeter(
            bool(self.settings.getSetting('show-fps', False)))

        musicVolume = _boundedNumber(
            self.settings.getSetting('music-volume', 0.65),
            0.65,
            0.0,
            1.0)
        sfxVolume = _boundedNumber(
            self.settings.getSetting('sfx-volume', 1.0),
            1.0,
            0.0,
            1.0)
        if self.musicManagerIsValid:
            self.musicManager.setVolume(musicVolume)
        for index, manager in enumerate(self.sfxManagerList):
            if self.sfxManagerIsValidList[index]:
    ...
    def _logMeasuredFrameRate(self, task):
        self.notify.info(
            'Measured client frame rate: %.1f FPS' %
