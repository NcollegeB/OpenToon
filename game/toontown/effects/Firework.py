# This module builds and controls firework visual, interval, or particle effects for visual,
    ...
    def generateFireworkIval(self):
        if not self.fireworkIval:
            ...
            if self.typeId == FireworkType.BasicPeony:
                ...
                self.fireworkEffects.append(firework)
                self.fireworkIval.append(firework.getFireworkMainIval())
            elif self.typeId == FireworkType.TripleBurst:
                leftBurst = FireworkEffect(FireworkBurstType.PeonyShell, FireworkTrailType.Glow, velocity=Vec3(-50 * self.scale, 0, 100 * self.scale), scale=self.scale / 1.5, primaryColor=self.primaryColor, secondaryColor=self.secondaryColor, burstDelay=1.6)
                centerBurst = FireworkEffect(FireworkBurstType.PeonyShell, FireworkTrailType.Glow, velocity=Vec3(0, 0, 100 * self.scale), scale=self.scale / 1.5, primaryColor=self.primaryColor, secondaryColor=self.secondaryColor, burstDelay=1.6)
                rightBurst = FireworkEffect(FireworkBurstType.PeonyShell, FireworkTrailType.Glow, velocity=Vec3(50 * self.scale, 0, 100 * self.scale), scale=self.scale / 1.5, primaryColor=self.primaryColor, secondaryColor=self.secondaryColor, burstDelay=1.6)
                leftBurst.reparentTo(self)
                centerBurst.reparentTo(self)
                rightBurst.reparentTo(self)
                self.fireworkEffects = self.fireworkEffects + [leftBurst, centerBurst, rightBurst]
                fireworkParallel = Parallel()
                fireworkParallel.append(leftBurst.getFireworkMainIval())
                fireworkParallel.append(centerBurst.getFireworkMainIval())
                fireworkParallel.append(rightBurst.getFireworkMainIval())
