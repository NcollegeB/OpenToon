...
class TownBattle(StateData.StateData):
    ...
    def enter(self, event, parentFSMState, bldg = 0, creditMultiplier = 1, tutorialFlag = 0):
        ...
        self.creditLevel = None
        if (bldg and
                base.localAvatar.inventory.battleCreditMultiplierIsAbsolute()):
            self.creditMultiplier = (
                base.localAvatar.inventory.getBattleCreditMultiplier())
        else:
            self.creditMultiplier = (
                getBaseGagExperienceMultiplier() * creditMultiplier)
        self.tutorialFlag = tutorialFlag
        base.localAvatar.inventory.setBattleCreditMultiplier(
            self.creditMultiplier, absolute=True)
