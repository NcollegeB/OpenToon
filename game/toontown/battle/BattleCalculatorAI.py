...
class BattleCalculatorAI:
    ...
    def __init__(self, battle, tutorialFlag=0):
        ...
        self.delayedUnlures = []
        self.__skillCreditMultiplier = getBaseGagExperienceMultiplier()
        self.tutorialFlag = tutorialFlag
        self.trainTrapTriggered = False

    def setSkillCreditMultiplier(self, mult):
        self.__skillCreditMultiplier = (
            getBaseGagExperienceMultiplier() *
            mult *
            self.__getGlobalSkillCreditMultiplier())

    def setSkillCreditMultiplierAbsolute(self, mult):
        self.__skillCreditMultiplier = (
            mult * self.__getGlobalSkillCreditMultiplier())

    def __getGlobalSkillCreditMultiplier(self):
        mult = 1
        air = getattr(simbase, 'air', None)
        if air:
            invasionManager = getattr(air, 'suitInvasionManager', None)
            if invasionManager and invasionManager.getInvading():
                mult *= getInvasionMultiplier()
            holidayManager = getattr(air, 'holidayManager', None)
            if holidayManager and holidayManager.isMoreXpHolidayRunning():
