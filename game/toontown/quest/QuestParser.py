...
class NPCMoviePlayer(DirectObject.DirectObject):
    ...
    def __unloadChar(self, char):
        char.removeActive()
        if getattr(char.style, 'name', None) in ('mk', 'mn'):
            char.stopEarTask()
        char.delete()
    ...
    def play(self):
        ...
        for line in lines:
            ...
            if self.isLocalToon:
                ...
                elif command == 'LOAD_TOON_GUIDE':
                    self.parseLoadToonGuide(line)
    ...
    def parseLoadToonGuide(self, line):
        token, name = line
        guide = Toon.Toon()
        guide.setName(TTLocalizer.TutorialGuide)
        guide.setPickable(0)
        guide.setPlayerType(NametagGroup.CCNonPlayer)
        dna = ToonDNA.ToonDNA()
        dna.newToonRandom(84217, 'm', 1)
        # The tutorial script uses pointing animations that exist only for the
        # medium legacy Toon body in this resource pack.
        dna.updateToonProperties(torso='ms', legs='m')
        guide.setDNAString(dna.makeNetString())
        guide.animFSM.request('neutral')
        guide.uniqueName = lambda event: '%s-guide-%s' % (
            self.uniqueId,
            event,
        )
        guide.nametag.manage(base.marginManager)
        guide.addActive()
        guide.hideName()
        self.setVar(name, guide)
        self.chars.append(guide)
...
if __debug__:
    searchPath.appendDirectory(
        Filename('open_town_assets/phase_3/etc')
    )
