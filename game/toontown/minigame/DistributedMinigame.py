# This module implements the client-side distributed minigame, handling network updates,
...
class DistributedMinigame(DistributedObject.DistributedObject):
    ...
    def delete(self):
        ...
        self.__destroySkipMinigameButton()
    ...
    def __createSkipMinigameButton(self):
        self.__destroySkipMinigameButton()
        self.skipMinigamePending = False
        self.skipMinigameButton = DirectButton(
            parent=base.a2dBottomRight,
            relief=DGG.RAISED,
            frameColor=(0.2, 0.24, 0.3, 0.95),
            frameSize=(-3.7, 3.7, -0.75, 1.05),
            borderWidth=(0.08, 0.08),
            text=TTLocalizer.MinigameSkipButton,
            text_fg=(1, 1, 1, 1),
            text_scale=0.7,
            text_pos=(0, 0.03),
            pos=(-0.32, 0, 0.14),
    ...
    def __destroySkipMinigameButton(self):
        ...
        if self.skipMinigameButton:
            self.skipMinigameButton.destroy()
            self.skipMinigameButton = None
        if self.skipMinigameStatus:
            self.skipMinigameStatus.destroy()
            self.skipMinigameStatus = None
        self.skipMinigamePending = False
    ...
    def __requestSkipMinigame(self):
        ...
        self.skipMinigameConfirm = TTDialog.TTDialog(
            ...
            text=TTLocalizer.MinigameSkipConfirm,
            text_wordwrap=18,
            fadeScreen=1,
            command=self.__handleSkipMinigameConfirm)
        self.skipMinigameConfirm.show()

    def __handleSkipMinigameConfirm(self, buttonValue):
        if self.skipMinigameConfirm:
            self.skipMinigameConfirm.cleanup()
            self.skipMinigameConfirm = None
        if (
        ...
        if self.skipMinigameButton:
            self.skipMinigameButton['state'] = DGG.DISABLED
        if self.skipMinigameStatus:
            self.skipMinigameStatus['text'] = (
                TTLocalizer.MinigameSkipVoteSubmitted)
        self.d_requestExit()

    def setSkipVoteStatus(self, voteCount, requiredCount):
        if not self.hasLocalToon or not self.skipMinigameStatus:
            return
        if requiredCount <= 0:
            self.skipMinigameStatus['text'] = ''
            return
        if voteCount >= requiredCount:
            self.skipMinigameStatus['text'] = (
                TTLocalizer.MinigameSkipVoteAccepted)
        else:
            self.skipMinigameStatus['text'] = (
                TTLocalizer.MinigameSkipVoteStatus %
                (voteCount, requiredCount))
    ...
    def enterFrameworkGame(self):
        ...
        self.__createSkipMinigameButton()
