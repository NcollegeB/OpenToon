...
class DistributedMinigameAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def gameOver(self):
        ...
        self.__grantCompletedMinigameQuestCredit()
    ...
    def __grantCompletedMinigameQuestCredit(self):
        if not MinigameSkipPolicy.shouldGrantQuestCredit(
                self.normalExit,
                self.explicitSkip,
                self.questCreditGranted):
            return
        toons = []
        for avId in self.avIdList:
            ...
            if toon is not None:
                toons.append(toon)
        for toon in toons:
            self.air.questManager.toonPlayedMinigame(toon, toons)
        self.questCreditGranted = True

    def __sendSkipVoteStatus(self):
        self.sendUpdate(
            'setSkipVoteStatus',
            [len(self.skipVotes), len(set(self.avIdList))])
    ...
    def requestExit(self):
        ...
        if avId not in self.avIdList:
            self.air.writeServerEvent(
                'suspicious',
                avId,
                'requested minigame skip but is not a participant in %s' %
                self.minigameId)
            return
        state = self.frameworkFSM.getCurrentState()
        if not state:
            return
        ...
        if not MinigameSkipPolicy.canRequestSkip(stateName, gameState):
            self.notify.warning(
                'BASE: ignoring minigame skip from %s in states %s/%s' %
                (avId, stateName, gameState))
            return
        if self.explicitSkip:
            return
        accepted, added, voteCount, requiredCount = (
            MinigameSkipPolicy.recordSkipVote(
                self.skipVotes, avId, self.avIdList))
        if not added:
            self.__sendSkipVoteStatus()
            return
        self.air.writeServerEvent(
            'minigame_skip_vote',
            avId,
        ...
        if not accepted:
            return
        self.explicitSkip = True
        self.air.writeServerEvent(
            'minigame_skipped',
            avId,
            '%s|%s|unanimous' % (self.minigameId, self.trolleyZone))
        self.notify.debug(
            'BASE: requestExit: unanimous skip accepted after vote by %s' %
            avId)
        self.setGameAbort()
