...
class DistributedPatternGame(DistributedMinigame):
    ...
    def load(self):
        ...
        hostArrow = matchingGameGui.find('**/minnieArrow')
        hostX = matchingGameGui.find('**/minnieX')
        hostCircle = matchingGameGui.find('**/minnieCircle')
        self.arrows = [None] * 5
        for x in range(0, 5):
            self.arrows[x] = hostArrow.copyTo(base.hidden)
            self.arrows[x].hide()

        self.xs = [None] * 5
        for x in range(0, 5):
            self.xs[x] = hostX.copyTo(base.hidden)
            self.xs[x].hide()
        ...
        for x in range(0, 4):
            for y in range(0, self.totalMoves):
                self.statusBalls[x][y] = hostCircle.copyTo(base.hidden)
                self.statusBalls[x][y].hide()

        hostArrow.removeNode()
        hostX.removeNode()
        hostCircle.removeNode()
        matchingGameGui.removeNode()
        self.host = Toon.Toon()
        dna = ToonDNA.ToonDNA()
        dna.newToonRandom(19324, 'f', 1)
        self.host.setDNAString(dna.makeNetString())
        self.host.setName(TTLocalizer.PatternGameHost)
        self.host.setPickable(0)
        self.host.reparentTo(base.hidden)
        self.backRowHome = Point3(3, 11, 0)
        self.backRowXSpacing = 1.8
        self.frontRowHome = Point3(0, 18, 0)
        self.frontRowXSpacing = 3.0
        self.stdNumDanceStepPingFrames = self.host.getNumFrames(self.hostAnimNames[0])
    ...
    def onstage(self):
        ...
        for anim in self.hostAnimNames:
            self.host.pose(anim, 0)

        for anim in self.toonAnimNames:
            self.lt.pose(anim, 0)

        self.hostAnimSpeedMult = {}
        self.toonAnimSpeedMult = {}
        for anim in self.hostAnimNames:
            numFrames = self.host.getNumFrames(anim)
            self.hostAnimSpeedMult[anim] = float(self.__numPingPongFrames(numFrames)) / float(self.stdNumDanceStepPingPongFrames)
        ...
        m.loop('neutral')
        m.startBlink()
        self.host.nametag.manage(base.marginManager)
        self.host.startLookAround()
        self.host.nametag.getNametag3d().setChatWordwrap(8)
        self.arrowDict['m'] = [self.arrows.pop(), self.xs.pop()]
        jj = self.host.nametag3d
        for k in range(0, 2):
    ...
    def offstage(self):
        ...
        self.roundText.hide()
        self.host.nametag.unmanage(base.marginManager)
        self.host.stopLookAround()
        self.host.stop()
        self.host.stopBlink()
        self.host.reparentTo(base.hidden)
        self.lt.setScale(1)
    ...
    def getDanceStepDuration(self):
        ...
        return numFrames / abs(self.animPlayRate * self.hostAnimSpeedMult[self.hostAnimNames[0]] * self.host.getFrameRate(self.hostAnimNames[0]))
    ...
    def __getHostDanceStepAnimTrack(self, host, direction):
        animName = self.hostAnimNames[direction]
        return self.__getDanceStepAnimTrack(host, animName, self.hostAnimSpeedMult[animName])
    ...
    def getDanceSequenceAnimTrack(self, toon, pattern):
        ...
        if toon == self.host:
            getDanceStepTrack = self.__getHostDanceStepAnimTrack
    ...
    def __setHostChat(self, str, giggle):
        str = str.replace('%s', self.getAvatar(self.localAvId).getName())
        self.host.setChatAbsolute(str, CFSpeech)
        if giggle:
            dialogue = self.host.getDialogueArray()
            if dialogue:
                base.playSfx(dialogue[0])

    def __clearHostChat(self):
        self.host.clearChat()
    ...
    def enterShowServerPattern(self):
        ...
        for anim in self.hostAnimNames:
            self.host.setPlayRate(self.animPlayRate * self.hostAnimSpeedMult[anim], anim)

        text = self.strWatch
        danceTrack = self.getDanceSequenceAnimTrack(self.host, self.__serverPattern)
        ...
        self.showTrack = Sequence(Func(self.__setHostChat, text, 1), Wait(0.5), Parallel(danceTrack, soundTrack, arrowTrack), Wait(0.2), Func(self.__clearHostChat), Func(self.gameFSM.request, 'getUserInput'))
    ...
    def enterGetUserInput(self):
        ...
        self.setupTrack = Sequence(Func(self.__setHostChat, self.strGo, 0), Func(self.setText, self.roundText, TTLocalizer.PatternGameGo), Func(self.roundText.setScale, 0.3), Func(enableKeys), Func(startTimer), Wait(0.8), Func(self.__clearHostChat), Func(self.setText, self.roundText, ' '), Func(self.roundText.setScale, 0.12), Func(self.setTextFG, self.roundText, self.normalTextColor))
    ...
    def enterPlayBackPatterns(self):
        ...
        textTrack = Sequence(Wait(0.2), Func(self.__setHostChat, text, 0), Wait(1.3), Func(self.__clearHostChat))
    ...
    def enterCheckGameOver(self):
        ...
        if self.round < PatternGameGlobals.NUM_ROUNDS:
            ...
            if self.celebrate:
                ...
                self.__winTrack = Sequence(Func(self.__setHostChat, text, 1), Func(base.playSfx, self.perfectSound), Sequence(self.returnCelebrationIntervals(1)), Sequence(self.returnCelebrationIntervals(0)), Func(self.__clearHostChat), Func(self.gameOver))
            else:
                self.__winTrack = Sequence(Func(self.__setHostChat, text, 1), Func(base.playSfx, sound), Wait(delay), Func(self.__clearHostChat), Func(self.gameOver))
