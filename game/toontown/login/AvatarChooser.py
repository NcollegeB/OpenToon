...
class AvatarChooser(StateData.StateData):
    ...
    def enter(self):
        ...
        keyboardShortcutManager.registerEscape(
            self, self.__handleQuit, priority=20)
    ...
    def load(self, isPaid):
        ...
        self.pickAToonBackdrop = DirectFrame(
            parent=render2d,
            relief=DGG.FLAT,
            frameColor=(0.06, 0.34, 0.6, 1),
            frameSize=(-1, 1, -1, 1),
            state=DGG.DISABLED,
            sortOrder=DGG.BACKGROUND_SORT_INDEX - 10)
        self.pickAToonBackdrop.setBin(
            'fixed', DGG.BACKGROUND_SORT_INDEX - 10)
        self.pickAToonBackdrop.hide()
        self.pickAToonBG = newGui.find('**/tt_t_gui_pat_background')
        self.pickAToonBG.reparentTo(hidden)
        # The selectable panels use aspect2d, so the background must use the
        # same coordinate system to stay aligned on widescreen displays.  The
        # solid backdrop fills the extra space outside the original 4:3 art.
        self.pickAToonBG.setPos(0.0, 0.0, 0.0)
        self.pickAToonBG.setScale(1, 1, 1)
        self.pickAToonBG.setDepthTest(False)
        self.pickAToonBG.setDepthWrite(False)
        self.pickAToonBG.setBin('fixed', DGG.BACKGROUND_SORT_INDEX)
    ...
    def unload(self):
        ...
        self.pickAToonBackdrop.destroy()
        del self.pickAToonBackdrop
