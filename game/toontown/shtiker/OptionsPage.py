# This module implements the options interface, presenting and updating player controls and
...
from . import GraphicsOptionsDialog
...
class OptionsTabPage(DirectFrame):
    ...
    def load(self):
        ...
        self.graphicsOptions = None
        self.confirm = None
        self._pendingExitAction = None
        ...
        self.graphicsOptionsButton = DirectButton(parent=self, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=1.15, text='Advanced Settings', text_scale=0.043, text_pos=button_textpos, textMayChange=0, pos=(-0.58, 0, -0.6), command=self.__doGraphicsOptions)
        self.mainMenuButton = DirectButton(parent=self, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=1.15, text=TTLocalizer.OptionsPageMainMenu, text_scale=0.043, text_pos=button_textpos, textMayChange=0, pos=(0, 0, -0.6), command=self.__handleMainMenuShowWithConfirm)
        self.exitButton = DirectButton(parent=self, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=1.15, text=TTLocalizer.OptionsPageExit, text_scale=options_text_scale, text_pos=button_textpos, textMayChange=0, pos=(0.58, 0, -0.6), command=self.__handleExitShowWithConfirm)
    ...
    def exit(self):
        ...
        if self.confirm != None:
            ...
            self.confirm = None
            self._pendingExitAction = None
        if self.graphicsOptions != None and self.graphicsOptions.isEntered:
            self.graphicsOptions.exit()
        self.hide()
    ...
    def unload(self):
        ...
        self.displaySettings = None
        if self.graphicsOptions != None:
            self.graphicsOptions.unload()
        self.graphicsOptions = None
        self.exitButton.destroy()
        self.mainMenuButton.destroy()
        self.graphicsOptionsButton.destroy()
        ...
        del self.exitButton
        del self.mainMenuButton
        del self.graphicsOptionsButton
    ...
    def __doGraphicsOptions(self):
        messenger.send('wakeup')
        if self.graphicsOptions == None:
            self.graphicsOptions = GraphicsOptionsDialog.GraphicsOptionsDialog()
            self.graphicsOptions.load()
        self.graphicsOptions.enter()
    ...
    def __showExitConfirm(self, action, message):
        if self.confirm != None:
            return
        self._pendingExitAction = action
        self.confirm = TTDialog.TTGlobalDialog(
            doneEvent='confirmDone', message=message,
            style=TTDialog.TwoChoice)
        self.confirm.show()
        self.accept('confirmDone', self.__handleConfirm)

    def __handleMainMenuShowWithConfirm(self):
        self.__showExitConfirm(
            'mainMenu', TTLocalizer.OptionsPageMainMenuConfirm)

    def __handleExitShowWithConfirm(self):
        self.__showExitConfirm('exit', TTLocalizer.OptionsPageExitConfirm)

    def __handleConfirm(self):
        status = self.confirm.doneStatus
        action = self._pendingExitAction
        self.ignore('confirmDone')
        self.confirm.cleanup()
        self.confirm = None
        self._pendingExitAction = None
